...
class FitnessGoalController extends ChangeNotifier {
  ...
  Future<void> saveFitnessGoalDetails(BuildContext context) async {
    try {
      ...
      await FirebaseFirestore.instance.collection('UserDataCollection').doc(user!.uid).set(fitnessGoalData, SetOptions(merge: true));

      //await SharedPreferencesHelper.setGoalCompleted(true);
